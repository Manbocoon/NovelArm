            ...
            using (Updater updater = new Updater())
            {
                ...
                if (double.TryParse(appInfo.Version, out double newVersion))
                {
                    if (Program.VERSION < newVersion)
                    {
                        ...
                        DialogResult userAction = MessageBox.Show($"[패치 노트]\n{appInfo.PatchNote}\n\n{numberForFun}번 지구에서 {Program.APP_NAME} 최신버전이 등장했대요!\n훔쳐올 테니까 업데이트 하실래요?", "상태창", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
